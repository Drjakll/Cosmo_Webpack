...
class Messaging extends Component {
    ...
    Get_Conversations = async () => {
        ...
        if(data){
            ...
            this.setState({private_conversations: private_conv_json_format});
    ...
    Init_IO = ()=>{
        ...
        this.msg_socket?.on('connect', ()=>{
            ...
            this.msg_socket.emit('join_private_channels', this.state.private_conversations);
        ...
        this.msg_socket?.on('clear_seen_by', async ({room_tag, signal_sent_by})=>{
            ...
            let {private_conversations, account_data} = this.state;

            private_conversations[room_tag]?.seen_by = {};

            await this.setState({private_conversations});
        ...
        this.msg_socket?.on('update_seen_by', async ({room_tag, seen_by})=>{
            ...
            private_conversations[room_tag]?.seen_by[seen_by] = seen_by;
        ...
        this.msg_socket?.on('report_online', ({email, room_tag, massive_send_out})=>{
            ...
            private_conversations[room_tag].online_users[email] = true;

            this.setState({private_conversations});
        ...
        this.msg_socket?.on('report_offline', ({room_tag, email})=>{
            ...
            let {private_conversations} = this.state;

            delete private_conversations[room_tag].online_users[email];
    ...
    Send_Message = (msg)=>{
        ...
        let msg_obj = {from, msg, read_by: {}, timestamp: 0};
    ...
    Save_Conversation = async (current_room_tag)=>{
        ...
        let conversation_info = this.state.private_conversations[current_room_tag];
    ...
    Recieve_Msg = async (room_tag, msg_obj) => {
        ...
        private_conversations[room_tag].messages.push(msg_obj);
    ...
    Seen_By = async (room_tag)=>{
        ...
        let {selected_room_tag, account_data, private_conversations} = this.state;

        if(!selected_room_tag && !room_tag){
            return;
        }

        let {seen_by} = private_conversations[room_tag] || private_conversations[selected_room_tag] || {};

        if(!seen_by || seen_by[account_data.email] !== undefined){
            return;
        }

        this.msg_socket?.emit('update_seen_by', {room_tag: room_tag || selected_room_tag, seen_by: account_data.email});
    }
    ...
    Clear_Seen_By = ()=>{
        ...
        this.msg_socket?.emit('clear_seen_by', {room_tag: selected_room_tag, signal_sent_by: account_data.email});
    }

    Set_Public_Private = (public_private)=>{
                ...
                <div id="bottom-section">
                    ...
                    <div id="message-area-wrapper">
                        ...
                        <Message_Area 
                            connection_list={this.state.connection_list}
                            account_data={this.state.account_data}
                            private_conversations={this.state.private_conversations}
